<template>
  <div class="flex h-screen">
    ...
    <div class="w-64 bg-gray-900 p-4 flex flex-col gap-3 border-r border-gray-800 overflow-y-auto">
      ...
      <h3 class="text-gray-400 text-xs mt-1">视图导航</h3>
      <div class="flex flex-col gap-1">
        <button v-for="v in views" :key="v.key" @click="activeView = v.key"
          class="text-left px-3 py-2 rounded text-sm transition-colors"
          :class="activeView === v.key ? 'bg-orange-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'">
          {{ v.icon }} {{ v.label }}
        </button>
      </div>
    ...
    <!-- Main Dashboard -->
    <div class="flex-1 flex flex-col gap-3 p-4 overflow-y-auto bg-gray-950">
      <template v-if="activeView === 'dashboard'">
        <!-- Register Gauges -->
      ...
      <EnergyMonitorView v-else-if="activeView === 'energy'" />
    </div>
  </div>
</template>
...
import EnergyMonitorView from './components/EnergyMonitorView.vue'

const store = useModbusStore()
let timer: number | null = null
const activeView = ref<'dashboard' | 'energy'>('dashboard')

const views = [
  { key: 'dashboard' as const, label: '实时监控', icon: '📊' },
  { key: 'energy' as const, label: '能耗监测', icon: '⚡' }
]
